...
interface AddToFavoriteProps extends React.SVGProps<SVGSVGElement> {
    propertyId: string | number;
}

export const AddToFavorite = ({ propertyId, onClick }: AddToFavoriteProps) => {
    const { properties } = useAppSelector((state) => state?.favorite);
    const isFavorite = Boolean(properties?.find((property) => property?.id === Number(propertyId)));

    if (isFavorite)
        return (
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="cursor-pointer" onClick={onClick}>
                <path
